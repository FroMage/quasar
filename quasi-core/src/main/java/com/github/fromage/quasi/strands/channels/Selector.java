...
import com.github.fromage.quasi.strands.Synchronization;
...
public class Selector<Message> implements Synchronization {
    ...
    /**
     * Performs exactly one channel operation of a given set, blocking until any of the actions completes.
     * Same as calling {@link #select(boolean, com.github.fromage.quasi.strands.channels.SelectAction[]) select(false, actions)}.
     *
     * @param actions a list of actions, one of which will be performed.
     * @return the action that has completed successfully
     * @throws InterruptedException
    ...
    /**
     * Performs exactly one channel operation of a given set, blocking until any of the actions completes, but no longer than the given timeout.
     * Same as calling {@link #select(long, java.util.concurrent.TimeUnit, com.github.fromage.quasi.strands.channels.SelectAction[]) select(false, timeout, unit, actions)}.
     *
     * @param timeout the maximum duration to block waiting for an operation to complete.
     * @param unit    the time unit of the given timeout
     * @param actions a list of actions, one of which will be performed.
    ...
    /**
     * Attempts to performs exactly one channel operation of a given set if one can be completed without blocking.
     * This method only blocks if the listeners associated with the select actions block. Same as calling {@link #trySelect(boolean, com.github.fromage.quasi.strands.channels.SelectAction[]) trySelect(false, actions)}.
     *
     * @param actions a list of actions, one of which will be performed.
     * @return the action that has completed successfully
     */
    ...
    public Object register() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            lastRegistered = i;
            if (sa.isDone()) {
                assert winner == sa; // seen to have failed in com.github.fromage.quasi.strands.channels.GeneralSelectorTest > testFans1[5] 
                res = sa;
                break;
            } else {
                Object w = winner;
